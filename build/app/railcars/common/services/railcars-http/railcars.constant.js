/**
 * @author Maryna Duda
 * @email maryna.duda@innovinnpromm.com
 * @date 23.06.2016 11:26:17
 */
"use strict";
/**
 * @export RailcarListApi
 * @param {string} RailcarListApi Url api to ge get list of all railcars
 * @example 'api/railcar'
 */
// TODO: RailcarConstant | test data
//  export let RailcarListApi = 'assets/api/railcar.json';
exports.RailcarListApi = 'api/railcar';
/**
 * @export RailcarIdApi
 * @param {string} RailcarIdApi Url api to ge get railcarby id
 * @example 'api/railcar/%railcarID%';
 */
// TODO: RailcarConstant | test data
// export let RailcarIdApi   = 'assets/api/railcar/%railcarID%.json';
exports.RailcarIdApi = 'api/railcar/%railcarID%';
/**
 * @export RailcarIdDisparityApi
 * @param {string} RailcarIdDisparityApi Url api to ge get railcar disparity by id
 * @example 'api/railcar/%railcarID%/disparity';
 */
// TODO: RailcarConstant | test data
// export let RailcarIdDisparityApi   = 'assets/api/railcar/%railcarID%-disparity.json';
exports.RailcarIdDisparityApi = 'assets/api/railcar/%railcarID%-disparity.json';
/**
 * @export RailcarFileApi
 * @param {string} RailcarFileApi Url api to ge get railcar file
 * @example 'api/railcar/file';
 */
// TODO: RailcarConstant | test data
// export let RailcarFileApi   = 'assets/api/railcar/add.json';
exports.RailcarFileApi = 'assets/api/railcar/add.json';

//# sourceMappingURL=railcars.constant.js.map
